package mudkipboy7.game.render.renderers;
...
import mudkipboy7.game.CharacterSettings;
...
public class CharRenderer extends AbstractObjectRenderer {
	...
	public static final HashMap<Character, CharacterSettings> characters = new HashMap<Character, CharacterSettings>();;
	...
	private static void addCharToList(char character, int texId, float distadderMod) {
		CharacterSettings pair = new CharacterSettings(texId, distadderMod);
		characters.put(character, pair);
	}

	private static int getCharTexId(char character) {
		if (characters.containsKey(character)) {
			return characters.get(character).getTexId();
		}
		return 0;
	}
	...
	private static float getCharDistAdder(char character) {
		if (characters.containsKey(character)) {
			return characters.get(character).getDistAdderMod();
		}
		return 1.5F;
	}
